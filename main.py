...
# Риск менеджмент
MAX_DAILY_TRADES=3
DAILY_STOP_LOSS=3.0  # % от баланса
PARTIAL_CLOSE=0.5    # закрывать 50% на 1:1
TP1_RR=1.0           # первый тейк
TP2_RR=2.0           # второй тейк

# Telegram (заполните если хотите уведомления)
TELEGRAM_TOKEN=""
TELEGRAM_CHAT_ID=""
...
def signal(ex,symbol,trend,mtf_trend):
    ...
    volume_confirm=last["volume"] >= avg_vol * 1.5  # объём в 1.5x выше среднего
    sfp=sfp_pattern and volume_confirm
    ...
    if ote or sfp:
        if trend=="bullish":
            ...
            return {"side":"buy","entry":price,"sl":sl,"tp1":tp1,"tp2":tp2,
                    "symbol":symbol,"qty_closed":False}
        else:
            sl=h.iloc[-1]*1.001
            tp1=price-(sl-price)*TP1_RR
            tp2=price-(sl-price)*TP2_RR
            return {"side":"sell","entry":price,"sl":sl,"tp1":tp1,"tp2":tp2,
                    "symbol":symbol,"qty_closed":False}
    return None
...
class Trader:
    ...
    def can_trade(self):
        ...
        if self.daily_trades >= MAX_DAILY_TRADES:
            ...
            return False
        max_loss=self.bal*(DAILY_STOP_LOSS/100)
    ...
    def open(self,sig):
        ...
        self.pos={**sig,"qty":qty,"qty_half":qty/2,"qty_closed":False}
        self.daily_trades+=1
    ...
    def check(self,price,symbol):
        ...
        # Trailing stop — двигаем стоп в безубыток после TP1
        if not self.pos["qty_closed"]:
            ...
            if hit_tp1:
                ...
                self.pos["sl"]=entry  # стоп в безубыток
                self.pos["qty"]=qty/2  # остаток
                msg=(f"⚡ ЧАСТИЧНОЕ ЗАКРЫТИЕ 50%\n"
                     f"{symbol} | {s.upper()}\n"
                     f"TP1: {tp1:.4f} | PnL: +{pnl_half:.2f} USDT\n"
        ...
        if hit_tp2 or hit_sl:
            ...
            result="win" if hit_tp2 or (hit_sl and self.pos["qty_closed"]) else "loss"
            ...
            print(f"\n{msg}")
            send_telegram(f"🤖 SMC BOT\n{msg}")
            self.trades.append({"symbol":symbol,"side":s,"entry":entry,
                                 "exit":ep,"pnl":round(pnl,2),"result":result})
            self.pos=None
    ...
    def status(self):
        ...
        print(f"🛡 Дн. потери: {self.daily_loss:.2f} USDT / {self.bal*(DAILY_STOP_LOSS/100):.2f} лимит")
        print(f"{'─'*45}")
